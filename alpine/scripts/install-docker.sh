...
## What's this?
## Script to set up docker in a fresh alpine instance :D

# Install deps
apk add openrc docker docker-compose

# mount cgroup
grep -qxF 'cgroup /sys/fs/cgroup cgroup defaults 0 0' /etc/fstab \
  || echo 'cgroup /sys/fs/cgroup cgroup defaults 0 0' >> /etc/fstab

# get start script for docker
wget -q \
  https://gist.githubusercontent.com/NamesMT/0708cc223a3db878df0477159115a49b/raw/17f45c7f23d3ecd8125963d5171f046979cb8f28/alpine.docker.service.sh \
  -O "$HOME/alpine.docker.service.sh"
...
# start docker and verify its status
